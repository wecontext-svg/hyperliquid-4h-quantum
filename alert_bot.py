...
def get_run_status():
    """Returns nice status line: Ran at XX:XX UTC (GitHub delay: X min)"""
    now = datetime.utcnow()
    # Scheduled time = every hour at :05
    scheduled = now.replace(minute=5, second=0, microsecond=0)
    if now < scheduled:
        scheduled -= timedelta(hours=1)
    
    delay_minutes = int((now - scheduled).total_seconds() / 60)
    ran_at = now.strftime("%H:%M UTC")
    return f"\n\nRan at {ran_at} (GitHub delay: {delay_minutes} min)"
...
Chart → https://app.hyperliquid.xyz/trade/{symbol}{status}
    """.strip()

    requests.post(f"https://api.telegram.org/bot{TOKEN}/sendMessage",
                  json={"chat_id": CHAT_ID, "text": msg, "parse_mode": "HTML"})
    print(f"✅ Alert sent → {symbol} {status}")

if __name__ == "__main__":
    print(f"⚡ Quantum scan started @ {datetime.utcnow()}")
